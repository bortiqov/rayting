 ...
 */
class FileManagerModalSettings extends InputWidget
{
	/**
	 * @var string
	 */
	 ...
	 * @var string
	 */
	public $file_delete_url = '/file/file/delete';
...
JS;
		...
		if($this->value !== null && !empty($this->value)){
		    if (sizeof(explode(",", $this->value)))
			foreach (explode(",", $this->value) as $via_element){
				$file = File::findOne($via_element);
				if($file === null){ return false;}
				$this->selected[] = $file->id;
				$isImage = $file->getIsImage() ? "filemanager-sortable-item-image" : "";

				$this->selected_items_sortable[$via_element]['content'] = '<div data-id="' . $file->id . '" class="filemanager-sortable-item ' . $isImage . '"><img src="' . $file->getSrc('small') . '"/><i class="filemanager-sortable-item-icon glyphicon glyphicon-file"></i><span class="filemanager-sortable-item-name">' . Html::encode($file->title) . '</span><span class="filemanager-sortable-item-remove"><i class="glyphicon glyphicon-remove"></i></span></div>';

			}
		} else {
		    $file = File::findOne($this->model_db->{$this->relation_name}->{$this->file_key});
            if($file === null){ return false;}
            $this->selected[] = $file->id;
            $isImage = $file->getIsImage() ? "filemanager-sortable-item-image" : "";

            $this->selected_items_sortable[$this->model_db->{$this->relation_name}->{$this->file_key}]['content'] = '<div data-id="' . $file->id . '" class="filemanager-sortable-item ' . $isImage . '"><img src="' . $file->getSrc('small') . '"/><i class="filemanager-sortable-item-icon glyphicon glyphicon-file"></i><span class="filemanager-sortable-item-name">' . Html::encode($file->title) . '</span><span class="filemanager-sortable-item-remove"><i class="glyphicon glyphicon-remove"></i></span></div>';
        }
	...
	public function run()
    {
		...
		if($this->organization_id){
			$searchModel->organization_id = $this->organization_id;
		}
