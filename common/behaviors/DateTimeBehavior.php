...
class DateTimeBehavior extends AttributeBehavior
{
    ...
    public $disableActions = ['api\modules\manager\modules\v1\controllers'];
    ...
    {
        if (in_array($this->owner->scenario, $this->disableScenarios)) {
            return;
        }
    ...
    {
        if (in_array($this->owner->scenario, $this->disableScenarios) || in_array(Yii::$app->controller->module->controllerNamespace, $this->disableActions)) {
            return $this->owner->{$this->attribute};
        }
        $formatter = Yii::$app->formatter;
        $this->owner->{$this->attribute} = $formatter->asDatetime($this->owner->{$this->attribute}, $this->format);
    }
    ...
    public function dbDate()
    {
        ...
        if (strlen($this->owner->{$this->attribute}) == 4) {
            $this->owner->{$this->attribute} = $this->owner->{$this->attribute} . '-01-01';
        }
